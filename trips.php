...
if (checkPermission($database, 0) || checkPermission($database, 2) || checkPermission($database, 4)) {
  ...
  drawTable("SELECT tripActivityNo, description, cost, confirmed, startDate, endDate FROM tripActivities WHERE tripNo = $_POST[id]", $database, ["Activity ID", "Description", "Cost", "Confirmation", "Start Date", "End Date"], "activity", "tripActivityNo", "id");
}
else {
